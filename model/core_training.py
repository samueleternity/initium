...
def save_checkpoint(path, rnn, output_proj, stochastic_heads, optimizer,
    ...
    model_config = {
        "input_size": rnn.input_size, "hidden_size": MODEL_HIDDEN_SIZE,
...
def run(beta_target: float, run_id: str, seed: int = SEED, resume_from: str = None,
    ...
    if not resuming:
        log_writer.writerow([
            ...
            "moe_aux_loss", "moe_cv_importance", "moe_cv_load", "moe_max_load_frac",  
        ])
    ...
    while step < total_steps:
        ...
        if step % EVAL_EVERY == 0:
            ...
            ood_triple_acc, ood_perfect_frac, ood_hop_breakdown = dataset.evaluate_ood(
                rnn, device, num_episodes=OOD_EVAL_EPISODES_PERIODIC, rng=ood_rng,
            ...
            ood_log_file.flush()
            dataset.write_field_log(field_log_writer, field_log_file, step, curriculum.lesson + 1, "ood", ood_field_log)
        ...
        if step % checkpoint_every == 0 or step == total_steps:
            ...
            save_checkpoint(ckpt_path, rnn, output_proj, stochastic_heads,
                             ...
                             beta_target, run_id, scaler, ood_rng,  
                             controller_type=controller,  
                             link_matrix_mode=link_matrix_mode, link_matrix_topk=link_matrix_topk,  
    ...
    ood_triple_acc, ood_perfect_frac, ood_hop_breakdown = dataset.evaluate_ood(
        rnn, device, num_episodes=OOD_EVAL_EPISODES, rng=ood_rng, verbose_n=10,
    )
