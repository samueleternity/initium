...
class CachedInferenceEngine(InferenceEngine):
    def __init__(self, model, output_proj, task, device, caches, ablate_memory: bool = False,
        ...
        self.verify = {"checked": 0, "failures": 0, "max_output_diff": 0.0,
                       "max_state_diff": 0.0, "disabled_after_failure": False}
        self.notes: List[str] = []
    ...
    def _valid_boundaries(self, ep) -> List[int]:
        ...
        if len(ok) != len(bs) and not self._warned_bounds:
            ...
            self._note("some declared cache boundaries were dropped (out of range, or a scored "
    ...
    def _maybe_verify(self, ctx, incoming_hidden, out, new_hidden, event, state_valid):
        ...
        self._disabled = True
        self._note(f"VERIFICATION FAILED at episode {ctx.index} ({event}): max|d output|={d_out:.3e}, "
    ...
    def _run_one(self, ctx: EpisodeCtx, hidden, active):
        for c in active:
            ...
            if hit is not None:
                ...
                return self._maybe_verify(ctx, hidden, hit.output, new_hidden, f"hit:{c.name}",
        ...
        if resume is not None:
            ...
            out, new_hidden, event = self._maybe_verify(ctx, hidden, out, new_hidden, event,
                                                        state_valid=True)
    ...
    def run(self, episodes, reset_experience: bool, verbose_n: int = 0,
        ...
        for i, ep in enumerate(episodes):
            ...
            ctx = EpisodeCtx(
                index=i, episode=ep, reset_experience=reset_experience,
                in_hash=hash_tensor(ep.input_seq) if act else "", chain=chain,
