...
from memory_manipulation.stochastic_write_head_v2 import install_stochastic_write_heads, load_prior_state
...
def _mamba1(c):  return dict(mamba_d_state=c.get("mamba_d_state", cc.MAMBA_D_STATE),
                             mamba_d_conv=c.get("mamba_d_conv", cc.MAMBA_D_CONV),
                             mamba_expand=c.get("mamba_expand", cc.MAMBA_EXPAND))
def _mamba2(c):  return dict(mamba2_d_state=c.get("mamba2_d_state", cc.MAMBA2_D_STATE),
...
def _mamba3(c):  return dict(mamba3_d_state=c.get("mamba3_d_state", cc.MAMBA3_D_STATE),
...
def _cfc(c):     return dict(cfc_mode=c.get("cfc_mode", cc.CFC_MODE),
...
def _build_split_graph(c: _Cfg, input_dim, hidden, nr_cells, cell_size, read_heads, device):
    ...
    default_hp = (
        ...
        if variant.startswith("mamba3") else
        dict(d_state=cc.MAMBA2_D_STATE, d_conv=cc.MAMBA2_D_CONV, expand=cc.MAMBA2_EXPAND)
        if variant.startswith("mamba2") else
        dict(d_state=cc.MAMBA_D_STATE, d_conv=cc.MAMBA_D_CONV, expand=cc.MAMBA_EXPAND)
    )
    hp = c.get("split_graph_mamba_hparams", default_hp)
    cfc_kwargs = c.get("split_graph_cfc_kwargs", dict(
    ...
    return SplitGraphDNC(
        ...
        mamba_d_state=hp["d_state"], mamba_d_conv=hp["d_conv"], mamba_expand=hp["expand"],
...
def load_model(ckpt: dict, device: torch.device, deterministic_write: bool = False) -> LoadedModel:
    ...
    if raw.get("split_graph_enabled", False):
        ...
        rnn = MambaDNC(
            input_size=input_dim, hidden_size=hidden, rnn_type=controller, num_layers=1,
            nr_cells=nr_cells, cell_size=cell_size, read_heads=read_heads,
            batch_first=True, device=device, independent_linears=True,
            **_controller_kwargs(controller, c), **extra,
        ).to(device)
    ...
    if c.defaulted:
        print(f"[model_loader] WARNING: checkpoint config lacks {sorted(set(c.defaulted))}; "
