...
def _build_split_graph(c: _Cfg, input_dim, hidden, nr_cells, cell_size, read_heads, device):
    ...
    return SplitGraphDNC(
        ...
        moe_capacity_factor=_moe["moe_capacity_factor"],
        moe_load_balance_alpha=_moe["moe_load_balance_alpha"],
        moe_cfc_multi_source=c.get(
...
def load_model(ckpt: dict, device: torch.device, deterministic_write: bool = False) -> LoadedModel:
    ...
    rnn.eval()
    output_proj.eval()
    combiner_stage_kinds = (
        list(getattr(getattr(rnn, "combiner_wrapper", None), "stage_kinds", []) or []) or None
    )
    return LoadedModel(
        rnn=rnn,
