    ...
    def _build_fused_episode(
        ...
        for i, (k, v) in enumerate(facts):
            ...
            inputs.append(
                fused(
                    [
                        (lambda s=sub, k=k, v=v, pt=pt: s._encode_fact(k, v, pt, perturb=perturb, rng=rng))
        ...
        for i, k in enumerate(query_keys):
            pt = 1.0 if i == 0 else 0.0
            inputs.append(fused([(lambda s=sub, k=k, pt=pt: s._encode_query(k, pt)) for sub in self.subs]))
            target_digits.append([0] * (2 * NF))
            answer_mask.append(0)
        ...
        for i, k in enumerate(query_keys):
            ...
            pt = 1.0 if i == 0 else 0.0
            inputs.append(fused([(lambda s=sub, pt=pt: s._encode_answer(pt)) for sub in self.subs]))
