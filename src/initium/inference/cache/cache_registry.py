...
from initium.inference.cache.base_cache import BaseCache
from initium.inference.cache.keys import model_fingerprint
...
from initium.inference.cache.stores import DiskStore, MemoryStore, TieredStore
...
def parse_cache_spec(spec: str) -> list[str]:
    ...
    for part in s.replace("+", ",").split(","):
        ...
        if part not in CACHE_CLASSES:
            raise ValueError(
                f"unknown cache type {part!r}; expected off, all, or a comma list of "
...
def setup_caches(
    ...
    *,
    ...
    if sampled_writes and not allow_stochastic:
        notes.append(
            "caching disabled: this model samples its write vectors (beta>0 without "
    ...
    if sampled_writes:
        notes.append(
            "WARNING: caching a stochastic model -- cached entries are ONE random draw "
    ...
    fp = model_fingerprint(
        ckpt,
        {
            "deterministic_write": bool(deterministic_write),
