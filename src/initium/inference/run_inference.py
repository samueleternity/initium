...
from src.initium.inference import run_logging as rl  # noqa: E402
from src.initium.inference.cache.cache_config import (  # noqa: E402
    DEFAULT_CACHE,
    DEFAULT_CACHE_DISK_MB,
    DEFAULT_CACHE_RAM_MB,
    DEFAULT_VERIFY_HITS,
)
from src.initium.inference.cache.cache_registry import parse_cache_spec, setup_caches  # noqa: E402
from src.initium.inference.cache.cached_engine import CachedInferenceEngine  # noqa: E402
from src.initium.inference.capabilities import (  # noqa: E402
    IncompatibleModelError,
    detect_capabilities,
    require_dims_match,
    require_type_supported,
)
from src.initium.inference.checkpoint_io import describe_checkpoint, load_checkpoint  # noqa: E402
from src.initium.inference.engine import InferenceEngine  # noqa: E402
from src.initium.inference.inference_config import (  # noqa: E402
...
from src.initium.inference.metrics import adaptation_trend, aggregate, windowed  # noqa: E402
from src.initium.inference.model_loader import load_model  # noqa: E402
from src.initium.inference.tasks.task_registry import get_task  # noqa: E402
...
def parse_args(argv=None):
    ...
    p.add_argument(
        "--device", type=str, default=None, help="default: cuda:0 if available, else cpu"
    )
    p.add_argument(
        "--deterministic-write",
        action="store_true",
    ...
    g.add_argument(
        "--cache-ram-mb", type=float, default=DEFAULT_CACHE_RAM_MB, help="RAM tier size cap."
    )
    g.add_argument(
        "--cache-disk-mb", type=float, default=DEFAULT_CACHE_DISK_MB, help="disk tier size cap."
    )
    g.add_argument(
        "--cache-verify",
        type=int,
        default=DEFAULT_VERIFY_HITS,
    ...
    g.add_argument(
        "--cache-clear", action="store_true", help="wipe this model's disk cache before running."
    )
    g.add_argument(
        "--cache-allow-stochastic",
        action="store_true",
...
def main(argv=None) -> int:
    ...
    if reset:
        ...
        n = (
            ...
            else (args.num_episodes if args.num_episodes is not None else DEFAULT_NUM_EPISODES)
        )

    rng = random.Random(args.seed)
    perturbation = (
        {"severity": args.perturbation_severity} if args.perturbation_severity is not None else None
    )
    episodes = task.build_episodes(
        n, rng, perturbation=perturbation
    )  # materialized -> identical episodes for --compare-fresh
    ...
    if not args.no_cache:
        caches, cache_notes, cache_fp = setup_caches(
            ...
            ckpt=ckpt,
            device=device,
            sampled_writes=loaded.sampled_writes,
            deterministic_write=args.deterministic_write,
            ablate_memory=args.ablate_memory,
            cache_dir=args.cache_dir,
            ram_mb=args.cache_ram_mb,
            disk_mb=args.cache_disk_mb,
            clear=args.cache_clear,
            allow_stochastic=args.cache_allow_stochastic,
        )
    ...
    if caches:
        ...
        if any(c.wants_boundaries for c in caches) and not any(
            ep.cache_boundaries for ep in episodes
        ):
            cache_notes.append(
                "prefix cache is on but no episode declares a static prefix "
    ...
    if caches:
        ...
        engine = InferenceEngine(
            loaded.rnn, loaded.output_proj, task, device, ablate_memory=args.ablate_memory
        )

    t0 = time.time()
    print(f"[inference] running {n} episodes | reset_experience={reset}")
    results = engine.run(
        episodes, reset_experience=reset, verbose_n=args.verbose_n, progress_every=PROGRESS_EVERY
    )
    ...
    summary = {
        ...
        "controller": caps.controller_type,
        "supported_types": caps.supported_types,
        "capabilities_source": caps.source,
        "dataset_type": canon,
        "dataset_link": args.dataset_link,
        "task": task.describe(),
        "mode": "fresh" if reset else "persistent",
    ...
    if caches:
        summary["cache"] = {
            ...
            "model_fingerprint": cache_fp,
            "disk_dir": args.cache_dir,
            "report": engine.cache_report(reset=True),
            "verify": dict(engine.verify),
            "store": caches[0].store.info(),
            "notes": cache_notes + engine.notes,
        }
    elif cache_notes:
        summary["cache"] = {
            "requested": args.cache,
