...
def parse_args(argv=None):
    ...
    p.add_argument("--test-dataset-link", type=str, default=None,
...
def main(argv=None) -> int:
    ...
    if args.generate:
        ...
        for prompt_index, (prompt, expected, meta) in enumerate(prompts):
            ...
            with_memory = generate(
                loaded.rnn, loaded.output_proj, task, prompt, args.max_new_steps,
                device, temperature=args.temperature, top_k=args.top_k, ablate_memory=False,
            )
            torch.manual_seed(pair_seed)
            if torch.cuda.is_available():
                torch.cuda.manual_seed_all(pair_seed)
            without_memory = generate(
                loaded.rnn, loaded.output_proj, task, prompt, args.max_new_steps,
                device, temperature=args.temperature, top_k=args.top_k, ablate_memory=True,
            )
        ...
        summary = {
            ...
            "memory_attributable_generation_probe": 100.0 * (correct_on - correct_off) / max(count, 1),
        ...
        if not args.no_log:
            ...
            path = os.path.join(args.log_dir, f"generation_{os.path.basename(args.checkpoint)}.json")
    ...
    if args.ablate_combiner_stage:
        ...
        if not kinds:
            raise SystemExit(
                "[inference] ABORT: --ablate-combiner-stage requires a split-graph "
    ...
    if args.report_moe_stats:
        ...
        if not moe_layers:
            ...
            print(
                f"[inference] MoE routing diagnostics ({len(moe_layers)} sublayer(s), "
            ...
            for i, layer in enumerate(moe_layers):
                ...
                src_diag = (
                    ...
                    if hasattr(layer, "last_source_diagnostics")
                    else None
                )
                if src_diag:
                    for s_idx, s in enumerate(src_diag):
                        frac_str = ", ".join(
                            f"e{e}:{f:.2f}" for e, f in enumerate(s["expert_frac"])
                        )
                        print(
