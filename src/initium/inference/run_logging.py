    ...
    with open(path, "w", newline="") as f:
        ...
        for w in windows:
            wr.writerow(
                ...
                + [w["field_acc"].get(n, "") for n in names]
            )
...
def print_summary(s: dict) -> None:
    ...
    print(
        f"checkpoint : {s['checkpoint']} (step {s['checkpoint_step']}, controller {s['controller']})"
    )
    print(f"task       : {s['task']}")
    print(
        f"mode       : {s['mode']} (reset_experience={s['reset_experience']}) | "
    ...
    if s.get("windows"):
        print(
            "windows    : "
            + " | ".join(
                f"[{w['start_episode']}-{w['end_episode']}] {w['item_acc']:.1f}%"
                for w in s["windows"]
            )
        )
    a = s.get("adaptation")
    if a:
        print(
            f"adaptation : first {a['window']} eps {a['first_window_item_acc']:.2f}% -> "
    ...
    if fb:
        print(
            f"fresh base : item acc {fb['item_acc']:.2f}% | perfect {fb['perfect_frac']:.2f}% "
    ...
    if c:
        print(
            f"cache      : requested={c['requested']} | active={c['active'] or 'none'}"
        ...
        for label, key in (("run", "report"), ("fresh baseline", "baseline_report")):
            for name, r in (c.get(key) or {}).items():
                print(
                    f"  [{label}] {name}: {r['hits']}/{r['lookups']} hits ({r['hit_rate']:.0f}%) "
        ...
        if v and v["checked"]:
            print(
                f"  verify: {v['checked']} hit(s) recomputed uncached | max |d output| "
