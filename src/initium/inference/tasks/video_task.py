...
class VideoChainTask(BaseInferenceTask):
    ...
    def __init__(self, dataset_link=None, path_length_range=None, prepared_dataset=None, **kwargs):
        ...
        self._ds = prepared_dataset or VideoChainDataset(test_dataset_link=link)
        self.input_dim, self.output_dim = self._ds.input_dim, self._ds.output_dim
        self.query_range = (
            tuple(path_length_range) if path_length_range else self._ds.ood_query_range
        )
    ...
    def build_episodes(self, n: int, rng, perturbation=None) -> list[Episode]:
        severity = (
            (perturbation or {}).get("severity", 0.0) if isinstance(perturbation, dict) else 0.0
        )
        ...
        for _ in range(n):
            ...
            for i, (k, v) in enumerate(facts):
                inputs.append(
                    self._ds._encode_fact(
                        k, v, 1.0 if i == 0 else 0.0, perturb=(severity or None), rng=rng
                    )
                )
                targets.append([0] * (2 * NF))
                mask.append(0)
            ...
            for i, k in enumerate(query_keys):
                ...
                targets.append(
                    self._ds.codec.label_to_digit_targets(vals_map[k])
