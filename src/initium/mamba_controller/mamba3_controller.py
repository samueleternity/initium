...
try:
    ...
    _MAMBA3_IMPORT_ERROR = (
        "mamba3_controller.py needs mamba-ssm installed from GitHub main "
...
class Mamba3ControllerCell(nn.Module):
    def __init__(
        ...
        m = self.mamba3
        self.d_model, self.d_inner, self.nheads, self.headdim = (
            d_model,
            m.d_inner,
            m.nheads,
            m.headdim,
    ...
    def init_state(self, batch_size, device=None, dtype=None):  # dtype ignored: always fp32
        ...
        return (
            torch.zeros(batch_size, H, S, **f),
            torch.zeros(batch_size, H, P, N, **f),
            torch.zeros(batch_size, H, N, **f),
            torch.zeros(batch_size, H, P, **f),
        )
    ...
    def step(self, u, state):
        ...
        with torch.autocast(device_type=u.device.type, enabled=False):
            ...
            B = _rms(B, m.B_norm.weight, m.B_norm.eps).unsqueeze(1) + m.B_bias.float().squeeze(
                1
            )  # (b,H,N)
            C = _rms(C, m.C_norm.weight, m.C_norm.eps).unsqueeze(1) + m.C_bias.float().squeeze(1)
            angle_new = angle_state + ang_proj.unsqueeze(1) * DT.unsqueeze(
                -1
            )  # (b,H,S) cumsum(angle*DT)
...
class Mamba3ControllerBlock(nn.Module):
    def __init__(
        ...
        self.cell = Mamba3ControllerCell(
            d_model,
            d_state=d_state,
            expand=expand,
            headdim=headdim,
